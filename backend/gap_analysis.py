...
- missing_keywords and suggested_skills: before marking ANYTHING as missing, scan ALL sections — Skills list, Experience descriptions, RESUME RAW TEXT, GITHUB PROJECT names/descriptions/topics, and RESUME PROJECT tech stacks. Treat synonyms as matches (e.g. "Data Visualization" matches Tableau/Power BI/matplotlib; "Machine Learning Engineering" matches ML + Python + model deployment; "Cloud Computing" matches AWS/Azure/GCP). A skill found ANYWHERE in the profile or reasonably implied by their tools is NOT missing. Only flag it if it is genuinely absent from every section.
