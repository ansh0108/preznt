  ...
  return (
    <section style={{ borderBottom: `1px solid ${BD}`, padding: "24px 24px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {FONTS.map(({ label, font, serif }) => (
          <div key={label} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <label style={{ fontSize: 12, fontWeight: 500, color: T2, fontFamily: "var(--sans)" }}>{label}</label>
            <button style={{ width: "100%", background: BG1, border: `1px solid ${BD}`, borderRadius: 4, padding: "8px 12px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }}
              onMouseEnter={e => e.currentTarget.style.borderColor = "var(--accent-b)"}
              onMouseLeave={e => e.currentTarget.style.borderColor = BD}>
              <span style={{ fontFamily: serif ? "var(--serif)" : "var(--sans)", fontSize: 16, color: T1 }}>{font}</span>
...
function DensitySection({ density, setDensity }) {
  return (
    <section style={{ borderBottom: `1px solid ${BD}`, padding: "24px 24px" }}>
        ...
        {DENSITIES.map(d => {
          ...
          return (
            <button key={d} onClick={() => setDensity(d)}
              style={{ padding: "12px 0", border: active ? `1px solid ${P}` : `1px solid ${BD}`, borderRadius: 4, display: "flex", flexDirection: "column", alignItems: "center", gap: 8, cursor: "pointer", background: active ? "var(--accent-d)" : "transparent" }}>
              <div style={{ display: "flex", flexDirection: "column", gap }}>
                {[24, 24, 16].map((w, i) => <div key={i} style={{ width: w, height: 4, background: active ? P : BGOV, borderRadius: 100 }} />)}
              </div>
...
function DarkModeSection({ darkMode, setDarkMode }) {
  return (
    <section style={{ padding: "24px 24px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <div style={{ fontSize: 14, fontWeight: 600, color: T1, fontFamily: "var(--sans)" }}>Enable Dark Mode Toggle</div>
          <div style={{ fontSize: 12, color: T2, fontFamily: "var(--sans)" }}>Allow visitors to switch themes</div>
        </div>
        <div onClick={() => setDarkMode(!darkMode)}
          style={{ width: 40, height: 24, borderRadius: 100, padding: 2, cursor: "pointer", display: "flex", alignItems: "center", border: `1px solid ${BD}`, background: darkMode ? P : BGOV, justifyContent: darkMode ? "flex-end" : "flex-start" }}>
    ...
    <aside style={{ width: 340, flexShrink: 0, background: BG2, borderRight: `1px solid ${BD}`, display: "flex", flexDirection: "column", overflow: "hidden", zIndex: 10 }}>
      <div style={{ padding: 8, display: "flex", gap: 8, background: BG2, borderBottom: `1px solid ${BD}`, flexShrink: 0 }}>
        {["Layout", "Theme", "Content"].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{ flex: 1, padding: "6px 0", textAlign: "center", fontSize: 14, fontWeight: 600, cursor: "pointer", borderRadius: 4, fontFamily: "var(--sans)", background: activeTab === tab ? BG1 : "transparent", color: activeTab === tab ? P : T2, border: activeTab === tab ? `1px solid ${BD}` : "1px solid transparent" }}>
            {tab}
          </button>
        ))}
      </div>
      <div style={{ flex: 1, overflowY: "auto", paddingBottom: 48 }}>
        ...
        <DarkModeSection darkMode={darkMode} setDarkMode={setDarkMode} />
      </div>
    </aside>
  );
}
    ...
    <nav style={{ height: 96, padding: "0 48px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${BD}` }}>
      ...
      <ul style={{ display: "flex", alignItems: "center", gap: 32, listStyle: "none", margin: 0, padding: 0 }}>
        {["Selected Work", "Philosophy", "Journal"].map((item, i) => (
          <li key={item} style={{ fontFamily: "var(--sans)", fontSize: 14, fontWeight: 600, color: i === 0 ? T1 : T2, cursor: "pointer" }}>{item}</li>
        ))}
      </ul>
      <button style={{ fontFamily: "var(--sans)", fontSize: 14, fontWeight: 600, color: T1, background: "none", border: "none", borderBottom: `1px solid ${T1}`, paddingBottom: 2, cursor: "pointer" }}
  ...
  return (
    <header style={{ padding: "48px 48px", display: "flex", gap: 64, alignItems: "center" }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap }}>
        ...
        <h1 style={{ fontFamily: "var(--serif)", fontSize: 48, fontWeight: 700, letterSpacing: "-0.02em", lineHeight: 1.05, color: T1, margin: 0 }}>Crafting seamless experiences that bridge utility and elegance.</h1>
        <p style={{ fontFamily: "var(--sans)", fontSize: 18, lineHeight: 1.6, color: T2, maxWidth: 400, marginTop: 16 }}>Senior Product Designer specializing in zero-to-one enterprise applications and high-end editorial interfaces. Based in San Francisco.</p>
      </div>
      <div style={{ width: "45%", aspectRatio: "4/5", flexShrink: 0, overflow: "hidden", position: "relative" }}>
  ...
  return (
    <section style={{ padding: "64px 48px", display: "flex", flexDirection: "column", gap: 48, borderTop: `1px solid ${BD}` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        ...
        <a href="#" onClick={e => e.preventDefault()} style={{ fontFamily: "var(--sans)", fontSize: 14, fontWeight: 600, color: T2, textDecoration: "none" }}
          onMouseEnter={e => e.target.style.color = P} onMouseLeave={e => e.target.style.color = T2}>View all →</a>
      </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", width: "100%", overflow: "hidden", background: BG, fontFamily: "var(--sans)" }}>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <section style={{ flex: 1, background: BGH, display: "flex", justifyContent: "center", alignItems: "flex-start", overflowY: "auto", padding: 40, position: "relative" }}>
