...
// ─── RepoListItem ─────────────────────────────────────────────────────────────
function RepoListItem({ repo, isSelected, onToggle }) {
  return (
    <div onClick={() => onToggle(repo.url)}
      style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", borderBottom: "1px solid var(--line)", cursor: "pointer", background: isSelected ? "var(--accent-d)" : "transparent", transition: "background 0.12s" }}>
      <div style={{ width: 17, height: 17, borderRadius: 5, flexShrink: 0, border: `1.5px solid ${isSelected ? "var(--accent)" : "var(--line2)"}`, background: isSelected ? "var(--accent)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.12s" }}>
...
// ─── RepoListPanel ────────────────────────────────────────────────────────────
function RepoListPanel({ repos, selected, setSelected, username, onConfirm }) {
  const allSelected = selected.size === repos.length;
  const toggleAll = () => setSelected(allSelected ? new Set() : new Set(repos.map(r => r.url)));
  const toggleOne = (url) => { const s = new Set(selected); s.has(url) ? s.delete(url) : s.add(url); setSelected(s); };
  return (
    <div style={{ animation: "fadeUp 0.25s ease" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button onClick={toggleAll} style={{ background: "var(--bg3)", border: "1px solid var(--line2)", color: "var(--text2)", borderRadius: "var(--r-sm)", padding: "4px 10px", fontSize: 11.5 }}>
            {allSelected ? "Deselect all" : "Select all"}
          </button>
        </div>
      </div>
      <div style={{ maxHeight: 270, overflowY: "auto", border: "1px solid var(--line2)", borderRadius: "var(--r-md)", marginBottom: 12 }}>
        {repos.map(repo => <RepoListItem key={repo.url} repo={repo} isSelected={selected.has(repo.url)} onToggle={toggleOne} />)}
      </div>
...
function GithubRepoPicker({ onConfirm }) {
  ...
  return (
    <div>
      ...
      {repos.length > 0 && <RepoListPanel repos={repos} selected={selected} setSelected={setSelected} username={username} onConfirm={onConfirm} />}
    </div>
  );
}
