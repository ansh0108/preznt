        ...
        {fitResult.suggested_skills?.length > 0 && (
          <div style={{ background: BG2, border: `1px solid ${BD}`, borderRadius: 12, padding: "14px 16px" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {fitResult.suggested_skills.map((s, i) => <Pill key={i} color="#d97706">{s}</Pill>)}
            </div>
          </div>
        )}
          ...
          <div style={{ background: BG2, border: `1px solid ${BD}`, borderRadius: 12, padding: "14px 16px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {fitResult.quick_wins.map((w, i) => (
                <div key={i} style={{ display: "flex", gap: 8, fontSize: 13, color: T2, lineHeight: 1.5 }}>
                  <span style={{ color: P, flexShrink: 0, fontWeight: 700 }}>{i + 1}.</span>
                  {w}
                </div>
              ))}
            </div>
      ...
      {tab === "portfolio" && (
        <div style={{ padding: "18px 20px" }}>
          {c.is_temp ? (
            ...
            <a href={portfolioUrl} target="_blank" rel="noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 6, color: P, fontSize: 13, fontWeight: 600, textDecoration: "none", marginBottom: 12 }}>
              Open full portfolio <Icon name="external" size={13} color={P} />
            </a>
          )}
