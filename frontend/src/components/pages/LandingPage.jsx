...
function ProfileCardMock() {
  return (
    <div className="float" style={{ ...glass, borderRadius: 20, padding: 10, transform: "rotate(2deg)", transition: "transform 0.5s ease", cursor: "default", width: 260 }}
      onMouseEnter={e => e.currentTarget.style.transform = "rotate(0deg)"}
      onMouseLeave={e => e.currentTarget.style.transform = "rotate(2deg)"}
    >
      <div style={{ width: "100%", height: 240, borderRadius: 12, overflow: "hidden", background: "linear-gradient(160deg, rgba(129,140,248,0.08) 0%, rgba(129,140,248,0.15) 50%, rgba(129,140,248,0.08) 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <div style={{ width: 88, height: 88, borderRadius: "50%", background: "rgba(129,140,248,0.12)", border: "2px solid rgba(129,140,248,0.2)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16 }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, padding: "12px 8px 6px" }}>
        {["AI Engineer", "Python", "Machine Learning"].map(tag => (
          <span key={tag} style={{ background: BGFIX, border: "1px solid rgba(129,140,248,0.18)", color: P, borderRadius: 100, padding: "4px 11px", fontSize: 11, fontWeight: 600, fontFamily: "var(--sans)" }}>{tag}</span>
        ))}
      </div>
      <div style={{ margin: "4px 8px 8px", padding: "9px 12px", background: BG2, borderRadius: 10, display: "flex", alignItems: "center", gap: 8 }}>
...
function FloatingAIChip() {
  return (
    <div style={{ position: "absolute", bottom: 0, right: -10, ...glass, borderRadius: 12, padding: "12px 18px", display: "flex", alignItems: "center", gap: 10, boxShadow: "0 8px 24px -6px rgba(129,140,248,0.18)" }}>
      <div style={{ width: 34, height: 34, borderRadius: "50%", background: BGFIX, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ fontSize: 16 }}>🎯</span></div>
      <div>
...
function AmbientOrbs() {
  return (
    <>
      <div style={{ position: "absolute", top: -60, right: -60, width: 480, height: 480, borderRadius: "50%", background: "rgba(129,140,248,0.06)", filter: "blur(100px)", pointerEvents: "none", zIndex: 0 }} />
      <div style={{ position: "absolute", bottom: -80, left: -80, width: 400, height: 400, borderRadius: "50%", background: "rgba(129,140,248,0.06)", filter: "blur(100px)", pointerEvents: "none", zIndex: 0 }} />
    </>
  );
}

function HeroCopy({ onGetStarted, onViewExamples }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <h1 style={{ fontFamily: "var(--serif)", fontSize: 48, fontWeight: 700, letterSpacing: "-0.02em", lineHeight: 1.1, color: T1, maxWidth: 640, margin: 0 }}>
        The Future of Your Career,<br /><em style={{ color: P, fontStyle: "italic" }}>Curated by AI.</em>
      </h1>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", paddingTop: 16 }}>
        <button onClick={onGetStarted} style={{ background: P, color: "#fff", border: "none", borderRadius: 100, padding: "16px 32px", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "var(--sans)", boxShadow: "0 0 20px 4px rgba(129,140,248,0.20)", transition: "opacity 0.15s" }} onMouseEnter={e => e.currentTarget.style.opacity = "0.88"} onMouseLeave={e => e.currentTarget.style.opacity = "1"}>Start Building Free</button>
        <button onClick={onViewExamples} style={{ background: "transparent", color: T1, border: "1px solid var(--line2)", borderRadius: 100, padding: "16px 32px", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "var(--sans)", transition: "all 0.15s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = "var(--accent-b)"; e.currentTarget.style.background = BG1; }} onMouseLeave={e => { e.currentTarget.style.borderColor = "var(--line2)"; e.currentTarget.style.background = "transparent"; }}>See How It Works</button>
      </div>
    </div>
  );
}
  ...
  return (
    <section style={{ padding: "96px 40px 128px", maxWidth: 1280, margin: "0 auto", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "7fr 5fr", gap: 24, alignItems: "center", position: "relative", zIndex: 1 }}>
        <HeroCopy onGetStarted={onGetStarted} onViewExamples={onViewExamples} />
        <ProfileIllustration />
      </div>
    </section>
  );
...
const INTERVIEW_TYPES = [
  { label: "Behavioral", color: "var(--accent)" },
  { label: "Technical", color: "#0d9488" },
  { label: "System Design", color: "#d97706" },
];
  ...
  return (
    <div style={{ flex: 1, height: 192, background: BGC, borderRadius: 8, border: `1px solid ${BD}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 8, width: "80%" }}>
        {INTERVIEW_TYPES.map(({ label, color }) => (
          <div key={label} style={{ background: BG1, borderRadius: 8, padding: "10px 14px", border: `1px solid ${BD}`, display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 8, height: 8, borderRadius: "50%", background: color, flexShrink: 0 }} />
            <span style={{ fontSize: 13, fontWeight: 600, color: T2, fontFamily: "var(--sans)" }}>{label}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <section id="features" style={{ padding: "48px 40px", maxWidth: 1280, margin: "0 auto" }}>
      <div style={{ textAlign: "center", marginBottom: 64 }}>
        <h2 style={{ fontFamily: "var(--serif)", fontSize: 32, fontWeight: 600, letterSpacing: "-0.01em", color: T1, margin: "0 0 16px" }}>Everything You Need to Land the Role</h2>
        <p style={{ fontSize: 18, lineHeight: 1.6, color: T2, maxWidth: 640, margin: "0 auto", fontFamily: "var(--sans)" }}>From building your portfolio to acing the interview — Prolio's AI works from your actual resume and experience, not templates.</p>
      </div>
